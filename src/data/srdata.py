...
class SRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        if train:
            n_patches = args.batch_size_update * args.test_every
            n_images = len(args.data_train) * len(self.images_hr)
            if args.num_data < 800:
                n_images = args.num_data
                
            if n_images == 0:
                self.repeat = 0
            else:
                self.repeat = max(n_patches // n_images, 1)
            # self.repeat == 1 for PTQ
    ...
    def _scan(self):
        ...
        for f in names_hr:
            ...
            for si, s in enumerate(self.scale):
                if 'DIV2K' in self.dir_hr and 'test' not in self.dir_hr:
